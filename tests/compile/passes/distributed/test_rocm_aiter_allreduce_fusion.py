...
def unfused_allreduce_add_rms_quant_gemm(
    ...
    """Unfused baseline: 4 separate kernel launches."""
        ...
        for use_residual in [False, True]:
            ...
            if mode == "eager":
                for M in batch_sizes:
                    ...
                    # deadlock with aiter barriers still in-flight.
...
@pytest.mark.parametrize("dtype", [torch.float16])
@pytest.mark.parametrize("num_tokens", [4, 1024])
@pytest.mark.skipif(
    not (torch.cuda.is_available() and torch.version.hip),
    reason="ROCm AITER fusion pass only runs on ROCm (HIP)",
)
def test_rocm_aiter_fused_op_correctness(
    ...
    """Verify vLLM custom op wrappers produce correct output.
    """
